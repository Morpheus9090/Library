...
const AddBook = () => {
 ...
 const handleImageChange = (e) => {
  ...
  reader.readAsDataURL(file); // safely read only if file exists
};
  ...
  return (
    ...
    <div className="min-h-screen text-[#252422] bg-[#f5f5f5] px-4 md:px-12">
        ...
        Add Book to SETEC Library
      </h2>
        <form onSubmit={handleSubmit} className="w-full max-w-xl mx-auto flex flex-col justify-center items-center space-y-4 mt-10">
        ...
        <div className="flex flex-col w-full">
          ...
          <input
            ...
            placeholder="e.g. ទន្សាយសុីចេក"
            className="w-full px-3 py-1.5 md:py-2 text-[#252422] rounded-lg bg-white border border-gray-500"
          />
        </div>
        <div className="flex flex-col w-full">
          <label className="md:text-lg">Subtitle:</label>
          <input
            type="text"
            value={subtitle}
            onChange={(e) => setSubtitle(e.target.value)}
            placeholder="ខ្មោចចេកជ្វា"
            className="w-full px-3 py-1.5 md:py-2 text-[#252422] rounded-lg  bg-white border border-gray-500"
          />
        </div>
        <div className="flex flex-col w-full">
